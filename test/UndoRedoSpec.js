...
describe('Undo/Redo', () => {
  ...
  it('undoing and redoing adding first marker', async () => {
    ...
    expect(track.getState()).to.deep.equal([
      { version: 2, start: [44.974635142416496, 6.064453125000001], metadata: { node: 1 } },
    ]);
  });
  ...
  it('undoing and adding two marker', async () => {
    ...
    await track.addNode(L.TrackDrawer.node(
      L.latLng(44.974635142416496, 6.064453125000001),
      { metadata: { node: 1 } },
    ));
    await track.addNode(L.TrackDrawer.node(
